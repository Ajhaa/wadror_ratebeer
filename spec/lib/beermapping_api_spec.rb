...
describe "BeermappingApi" do
  describe "in case of a cache miss" do
    ...
    it "When HTTP GET returns no entrys, returns empty array" do
      canned_answer = <<-END_OF_STRING
                      <?xml version='1.0' encoding='utf-8' ?><bmp_locations><location><id></id><name></name><status></status><reviewlink></reviewlink><proxylink></proxylink><blogmap></blogmap><street></street><city></city><state></state><zip></zip><country></country><phone></phone><overall></overall><imagecount></imagecount></location></bmp_locations>
                      END_OF_STRING

      stub_request(:get, /.*jaas/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

      places = BeermappingApi.places_in("jaas")
    ...
    it "When HTTP GET returns multiple entries, all are parsed and returned" do
      ...
      expect(places.last.name).to eq("Kaleva Brewing Company")
    end
  end
end
